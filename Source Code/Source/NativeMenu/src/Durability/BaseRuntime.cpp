...
void Log(const char*s){std::ofstream f(LogPath(),std::ios::app);f<<GetTickCount64()<<','<<s<<",redirects="<<redirects.load()<<",refusals="<<refusals.load()<<'\n';}
 ...
 for(int partition=0;partition<5;partition++){
  ...
  for(long long cellIndex=0;cellIndex<slots;cellIndex++){
   ...
   for(int record=0;record<recordCount;record++){
    ...
    int relation=0;bool hostile=!human;if(human){if(!Read(state+0x728,relation))continue;hostile=relation==3;}auto&life=lives[state];if(!life.generation)life.generation=1;else if(life.lastScan+7<scanNumber)life.generation++;life.lastScan=scanNumber;if(zombie)zombies++;else if(hostile)hostileHumans++;else friendlyHumans++;float distance=Distance(camera,position);espRoster[state]={{state,complete,life.generation,position,human,hostile},distance,GetTickCount64()};
