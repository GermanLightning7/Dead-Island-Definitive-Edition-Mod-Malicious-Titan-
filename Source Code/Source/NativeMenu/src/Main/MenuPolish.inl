...
void MenuPanel(std::vector<D3DVertex>&v,float w,float h,float br,float bg,float bb,float ar,float ag,float ab,float tr,float tg,float tb){
 ...
 MenuRound(v,-204,75,198,405,w,h,.05f,.035f,.08f,1);
 AddRect(v,-208,1,716,2,w,h,ar,ag,ab,1);
 MenuRound(v,-190,17,35,35,w,h,.22f,.11f,.34f,1);AddText(v,-180,24,"D",2.1f,w,h,tr,tg,tb);
 AddText(v,-140,16,"Dead Titans OFFICIAL",2.4f,w,h,tr,tg,tb);
 AddText(v,-139,42,"DEAD ISLAND DEFINITIVE EDITION",.85f,w,h,.73f,.64f,.87f);
 MenuRound(v,475,8,26,26,w,h,.16f,.09f,.24f,1);AddText(v,484,17,"X",1.4f,w,h,tr,tg,tb);
}
