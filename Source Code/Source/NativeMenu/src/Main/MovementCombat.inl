...
float ModifiedSpeedCap(void* who,float parameter){
 auto caller=(uintptr_t)_ReturnAddress();RecordCaller(caller);float value=nativeSpeedCap(who,parameter),scale=runScale.load();
...
void ModifiedRun(void* who,float dt){
 nativeRun(who,dt);DoubleJumpFrame(who);PumpBlueprintChange(who);PumpSpawnItem(who);PumpKickLaunchInput(who);PumpFunMovement(who,dt);PumpNoclip(who,dt);float scale=runScale.load();if(scale==1.f||!LocalMove(who))return;
