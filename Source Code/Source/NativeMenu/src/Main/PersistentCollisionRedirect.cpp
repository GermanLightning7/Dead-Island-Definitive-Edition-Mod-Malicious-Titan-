...
LONG themeColors[6]={(LONG)0xF2161024,(LONG)0xFFF3ECFF,(LONG)0xFFB784F5,(LONG)0xFF704CA2,(LONG)0xFF392451,(LONG)0xFFCEAAFF};
...
void AddMenu(std::vector<D3DVertex>&v,float screenW,float screenH){
 ...
 AddRect(v,16,h-52,478,1,w,h,.31f,.20f,.42f,1);AddText(v,x+16,y+h-40,menuStatus,std::min(1.f,478.f/(6.f*std::max<size_t>(1,strlen(menuStatus)))),w,h,.83f,.74f,.94f);AddText(v,x+16,y+h-22,ammoEditField.load()>=0?"DIGITS TYPE   BACKSPACE EDIT   ENTER APPLY   ESC CANCEL":"CLICK KEY BAR TO BIND   RESET CLEARS KEY   MENU KEY CLOSES",1,w,h,.60f,.55f,.55f);
...
void HandleMenuInput(){
 ...
 if(menuOpen.load()){
  int page=menuPage.load();if(pgup&&!oldPgUp){page=MenuNextTab(page,-1);menuPage=page;menuIndex=0;}if(pgdown&&!oldPgDown){page=MenuNextTab(page,1);menuPage=page;menuIndex=0;}const int counts[16]={5,8,2,5,12,7,9,9,4,12,5,5,3,9,10,4};int count=counts[page],index=menuIndex.load();if(up&&!oldUp)index=(index+count-1)%count;if(down&&!oldDown)index=(index+1)%count;menuIndex=index;
