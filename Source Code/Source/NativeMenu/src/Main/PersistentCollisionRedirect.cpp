...
constexpr uintptr_t ImpactRva=0x37F520,RocketCreateIatRva=0xC7DA68,RocketActivateIatRva=0xC7D928,RocketRttiRva=0x12C9FA0,RocketInitRva=0x5FA450,RocketDetonateRva=0x5FA110;
...
struct D3DVertex{float x,y;float r,g,b,a;float u=.5f/1024.f,v=.5f/512.f;};
...
std::atomic_bool espHealthEnabled{true},espDistanceEnabled{true};
std::atomic<unsigned long long> espHealthDraws{0},espDistanceDraws{0};
...
std::atomic_bool godModeEnabled{false};
std::atomic_int customCash{10000};
std::atomic_int hotkeys[6]{{VK_F11},{VK_F5},{VK_F6},{VK_INSERT},{VK_END},{'F'}};
...
std::atomic<uintptr_t> cachedRocketWorld{0},cachedRocketControl{0};std::atomic<unsigned long long> cachedRocketAt{0};
...
LONG themeColors[6]={(LONG)0xF20A0612,(LONG)0xFFF3ECFF,(LONG)0xFF6B3FA0,(LONG)0xFF3D2359,(LONG)0xFF2A1640,(LONG)0xFFCEAAFF};
...
std::filesystem::path LogPath(){wchar_t p[MAX_PATH]{};GetEnvironmentVariableW(L"LOCALAPPDATA",p,MAX_PATH);auto d=std::filesystem::path(p)/L"JentaSpecialEdition"/L"CollisionRedirectPersistent";std::filesystem::create_directories(d);return d/L"persistent_collision_redirect.txt";}
...
namespace CameraLockOn{extern std::atomic_int targetSource;}
#include "AimTargeting.h"
...
struct NativeSettings{unsigned magic=0x53444944,version=6;LONG theme[6]{};LONG zombie=0,friendly=0,hostile=0,interval=8,distance=4000,style=0,keys[5]{};LONG fovEnabled=0,fovRadius=250,crosshairPriority=0,targetMarker=0,targetSnapline=0,espSnaplines=0;LONG kickLaunchKey='F';LONG lockOnTargetSource=0;LONG targetPriority=0,aimBodyPart=0;};
...
#include "CashEdit.inl"
bool GiveCashNative(int amount){uintptr_t stats=0;if(!ResolveStats(stats)){SetStatus("CASH REFUSED - LOAD A SAVE");return false;}bool rollback=true;bool ok=CashEdit::Apply(stats,amount,[](uintptr_t p,int&v){return Read(p,v);},[](uintptr_t p,int v){return Write(p,v);},rollback);SetStatus(ok?"CASH APPLIED AND VERIFIED":rollback?"CASH REFUSED - RANGE OR STATE CHANGED":"CASH ROLLBACK FAILED - CHECK LOG");if(!rollback)Log("CASH_TRANSACTION_ROLLBACK_FAILED");return ok;}
...
bool ValidateEspEntity(const EspEntity&entity,V&freshRoot,float* currentHealth=nullptr){uintptr_t owner=0;if(!Read(entity.state+0x58,owner)||owner!=entity.complete)return false;if(entity.generation<=0)return false;uintptr_t vtable=0;if(!Read(entity.state,vtable))return false;bool correct=entity.humanType?vtable==gameBase+HumanStateRva:(vtable==gameBase+ZombieState1Rva||vtable==gameBase+ZombieState2Rva);if(!correct)return false;unsigned char vitals[0x40]{};if(!ReadBytes(entity.state+0x6F8,vitals,sizeof(vitals)))return false;float health=0;memcpy(&freshRoot,vitals,12);memcpy(&health,vitals+0x3C,4);if(currentHealth)*currentHealth=health;uintptr_t ownerAfter=0;if(!Read(entity.state+0x58,ownerAfter)||ownerAfter!=entity.complete)return false;float drift=Distance(freshRoot,entity.root);return Finite(freshRoot)&&std::isfinite(health)&&health>0&&health<=1000000&&std::isfinite(drift)&&drift<=5.0f;}
...
namespace PlayerTracker { void UpdatePlayer(uintptr_t addr, bool local, V position); }
...
bool EnumerateAndSelect(){
 ...
 const bool useFov=fovAimEnabled.load(std::memory_order_acquire),useCrosshair=AimTargeting::priority.load(std::memory_order_acquire)==2,needProjection=useFov||useCrosshair;const float screenW=(float)presentWidth.load(),screenH=(float)presentHeight.load();V aimCamera{},aimRight{},aimUp{},aimBack{};float aimSx=0,aimSy=0;const bool projectionReady=!needProjection||(screenW>0&&screenH>0&&ReadD3DCamera(aimCamera,aimRight,aimUp,aimBack,aimSx,aimSy));
 ++scanNumber;Target selected{};float selectedScreenDistance=-1,selectedHealth=FLT_MAX;EspFrame esp{};unsigned zombies=0,hostileHumans=0,friendlyHumans=0,recordsSeen=0,rejected=0,fovProjected=0,fovEligible=0;uintptr_t container=level+0x358;
 for(int partition=0;partition<5;partition++){
  ...
  for(long long cellIndex=0;cellIndex<slots;cellIndex++){
   ...
   for(int record=0;record<recordCount;record++){
    ...
    int objectOffset=0;bool zombie=TypeContains(vtable,"ZombieAI",objectOffset),human=false;if(!zombie)human=TypeContains(vtable,"HumanAI",objectOffset);
    if(!zombie&&!human){
     int playerOffset=0;
     if(TypeContains(vtable,"PlayerDI",playerOffset)){
      uintptr_t playerObj=igs-(intptr_t)playerOffset;
      uintptr_t localPlayer=0;TrueGodPlayer(localPlayer);
      V pPos{};
      if(Read(playerObj+0x578,pPos)&&Finite(pPos)){
       PlayerTracker::UpdatePlayer(playerObj,playerObj==localPlayer,pPos);
      }
     }
     continue;
    }
    uintptr_t complete=igs-(intptr_t)objectOffset,state=0;
...
#include "CombatLock.inl"
unsigned char Hook(void*w,V*start,V*end,void*collision,void*ignore,char flags){
 auto caller=(uintptr_t)_ReturnAddress();RecordCaller(caller);
 if(caller-gameBase!=PrimaryReturnRva)return original(w,start,end,collision,ignore,flags);
 if(CombatLock::Wanted())return CombatLock::LockedRay(w,start,end,collision,ignore,flags);
 if(enabled.load())return CombatLock::StandaloneRay(w,start,end,collision,ignore,flags);
 pendingRedirectExplosion.valid=false;return original(w,start,end,collision,ignore,flags);
}
...
bool InstallImpact(){const unsigned char expected[8]={0x40,0x55,0x53,0x56,0x57,0x41,0x54,0x48};auto target=(void*)(gameBase+ImpactRva);if(memcmp(target,expected,sizeof(expected))){Log("IMPACT_INSTALL_BYTE_MISMATCH");return false;}auto init=MH_Initialize();if(init!=MH_OK&&init!=MH_ERROR_ALREADY_INITIALIZED){Log("IMPACT_INSTALL_INIT_FAILED");return false;}if(MH_CreateHook(target,(void*)&ImpactHook,(void**)&originalImpact)!=MH_OK){Log("IMPACT_MELEE_CREATE_FAILED");return false;}if(MH_EnableHook(target)!=MH_OK){Log("IMPACT_MELEE_ENABLE_FAILED");return false;}Log("IMPACT_INSTALL_OK");return true;}
bool InstallDebug() {
...
#include "SmoothFont.inl"
void ReleaseD3D(){SmoothFont::Release();if(d3dRaster)d3dRaster->Release();if(d3dDepth)d3dDepth->Release();if(d3dBlend)d3dBlend->Release();if(d3dVertices)d3dVertices->Release();if(d3dLayout)d3dLayout->Release();if(d3dPs)d3dPs->Release();if(d3dVs)d3dVs->Release();d3dRaster=nullptr;d3dDepth=nullptr;d3dBlend=nullptr;d3dVertices=nullptr;d3dLayout=nullptr;d3dPs=nullptr;d3dVs=nullptr;}
bool EnsureD3D(ID3D11Device *d) {
  if(d3dVs&&d3dPs&&d3dLayout&&d3dVertices&&d3dBlend&&d3dDepth&&d3dRaster&&SmoothFont::texture&&SmoothFont::sampler)return true;
  ReleaseD3D();
  static const char *vs="struct V{float2 p:POSITION;float4 c:COLOR;float2 uv:TEXCOORD;};struct O{float4 p:SV_Position;float4 c:COLOR;float2 uv:TEXCOORD;};O main(V v){O o;o.p=float4(v.p,0,1);o.c=v.c;o.uv=v.uv;return o;}";
  static const char *ps="Texture2D atlas:register(t0);SamplerState fontSampler:register(s0);float4 main(float4 p:SV_Position,float4 c:COLOR,float2 uv:TEXCOORD):SV_Target{float a=atlas.Sample(fontSampler,uv).a;return float4(c.rgb,c.a*a);}";
  ...
  D3D11_INPUT_ELEMENT_DESC il[] = {{"POSITION", 0, DXGI_FORMAT_R32G32_FLOAT, 0,
                                   ...
                                   {"COLOR", 0, DXGI_FORMAT_R32G32B32A32_FLOAT,
                                    0, 8, D3D11_INPUT_PER_VERTEX_DATA, 0},{"TEXCOORD",0,DXGI_FORMAT_R32G32_FLOAT,0,24,D3D11_INPUT_PER_VERTEX_DATA,0}};
  d->CreateInputLayout(il, 3, vb->GetBufferPointer(), vb->GetBufferSize(),
                       &d3dLayout);
  vb->Release();
  pb->Release();
  D3D11_BUFFER_DESC bd{};
  bd.ByteWidth = sizeof(D3DVertex) * 131072;
...
#define G(ch,a,b,c,d,e) case ch:{static const unsigned char g[5]={a,b,c,d,e};return g;}
 ...
 G('*',0x14,0x08,0x3e,0x08,0x14)
 G('/',0x40,0x20,0x10,0x08,0x04) G('>',0,0x41,0x22,0x14,0x08) G('_',0x40,0x40,0x40,0x40,0x40) G('+',0x08,0x08,0x3e,0x08,0x08) G(':',0,0x36,0x36,0,0) G('-',0x08,0x08,0x08,0x08,0x08) G('.',0,0x60,0x60,0,0)
 default:return blank;
 }
#undef G
}
float TextWidth(const char*text,float scale){return SmoothFont::WidthOf(text,scale);}
void AddText(std::vector<D3DVertex>&v,float x,float y,const char*text,float scale,float w,float h,float r,float g,float b){SmoothFont::Text(v,x,y,text,scale,w,h,r,g,b);}

using MeleeStateFn=int(*)();using MeleeSetFn=int(*)(int);using MeleeStopFn=void(*)();
MeleeStateFn meleeMenuState=nullptr;MeleeSetFn meleeMenuSet=nullptr;MeleeStopFn meleeMenuStop=nullptr;
...
void LoadMeleeMenu(){
 ...
 wchar_t*slash=wcsrchr(path,L'\\');if(!slash)return;*(slash+1)=0;
 if(wcscat_s(path,L"DideMeleeNative.dll"))return;
 HMODULE module=LoadLibraryW(path);if(!module){Log("MELEE_MENU_LOAD_FAILED");return;}
 meleeMenuState=(MeleeStateFn)GetProcAddress(module,"DideMeleeState");
 meleeMenuSet=(MeleeSetFn)GetProcAddress(module,"DideMeleeSet");
 meleeMenuStop=(MeleeStopFn)GetProcAddress(module,"DideMeleeStop");
 oneShotMenuState=(MeleeStateFn)GetProcAddress(module,"DideMeleeOneShotState");
 ...
 *(slash+1)=0;if(wcscat_s(path,L"DideMeleeDurability.dll"))return;
 HMODULE wear=LoadLibraryW(path);if(wear){wearMenuState=(MeleeStateFn)GetProcAddress(wear,"DideWearState");wearMenuSet=(MeleeSetFn)GetProcAddress(wear,"DideWearSet");}

}
void ToggleMeleeMenu(){
 if(!meleeMenuState||!meleeMenuSet){SetStatus("MELEE MODULE UNAVAILABLE");return;}
 int result=meleeMenuSet(meleeMenuState()==1?0:1);
 SetStatus(result==1?"MELEE SILENT AIM ENABLED":result==0?"MELEE SILENT AIM DISABLED":"MELEE MODULE STOPPED OR UNAVAILABLE");
}
void ToggleOneShotMelee(){
 if(!oneShotMenuState||!oneShotMenuSet){SetStatus("MELEE MODULE UNAVAILABLE");return;}
 int result=oneShotMenuSet(oneShotMenuState()==1?0:1);
 SetStatus(result==1?"ONE-SHOT MELEE ENABLED":result==0?"ONE-SHOT MELEE DISABLED":"MELEE MODULE STOPPED OR UNAVAILABLE");
}
void ToggleChainMelee(){
 if(!chainMenuState||!chainMenuSet){SetStatus("MELEE MODULE UNAVAILABLE");return;}
 int result=chainMenuSet(chainMenuState()==1?0:1);
 SetStatus(result==1?"CHAIN MELEE ENABLED":result==0?"CHAIN MELEE DISABLED":"MELEE MODULE STOPPED OR UNAVAILABLE");
}
#include "MouseMenu.inl"
#include "StaminaFeature.inl"
void PumpBlueprintChange(void*who);
void PumpSpawnItem(void*who);
void PumpKickLaunchInput(void*who);
void PumpNoclip(void*who,float dt);



#include "MovementCombat.inl"
#include "ExplosiveMelee.inl"
#include "CharacterPhysics.inl"
#include "NoclipFeature.inl"
#include "VehicleBoost.inl"
#include "KickLaunch.inl"
#include "HybridBlueprint.inl"
#include "WeaponModification.inl"
#include "GrapplingHook.inl"
#include "FunFeatures.inl"
#include "ReadableWeaponLabels.inl"
#include "ItemSpawner.inl"
#include "WaveSkipper.inl"
#include "CameraFeatures.inl"
#include "CameraLockOn.inl"
#include "Spinbot.inl"
#include "NoRecoil.inl"
#include "VisualToggles.inl"
#include "GraphicsToggles.inl"
#include "ChapterUnlock.inl"
#include "InventoryDispatch.inl"
#include "Crosshair.cpp"
#include "PlayerTracker.inl"

#include "EspVitals.inl"
#include "MenuAnimation.inl"
#include "MenuPolish.inl"
#include "MenuPresets.inl"
void AddMenu(std::vector<D3DVertex>&v,float screenW,float screenH){
 const size_t firstVertex=v.size();float w=510,h=MenuHeight();
 ...
 const int page=std::clamp(menuPage.load(),0,17);const char*pages[18]={"OVERVIEW","PLAYER","ARENA","MOVEMENT","AIM","CAMERA","ENEMY ESP","STYLE","HOTKEYS","FIREARMS","MELEE","CRAFTING","ITEMS","VISUALS","FUN","CONFIGS","ACTION KEYS","PLAYERS"};
 const char*mainRows[5]={"RESET DEFAULTS","UNLOAD","MARK ISSUE IN LOG","OPEN LOG FOLDER","TOGGLE MENU"};const char*configRows[4]={"DEFAULT - ALL OFF","PRESET 1","PRESET 2","PRESET 3"};const char*playerRows[8]={"GOD MODE","CASH AMOUNT","GIVE CUSTOM CASH","GIVE 1000 CASH","GIVE 200000 CASH","RESTORE GOD BYTES","UNLOCK ALL CHAPTERS","INFINITE FLASHLIGHT"};const char*movementRows[7]={"INFINITE STAMINA","JUMP HEIGHT","RUN SPEED","SWING SPEED","INFINITE AIR JUMPS","NOCLIP","NOCLIP SPEED"};const char*arenaRows[2]={"SKIP CURRENT WAVE","CANCEL WAVE SKIP"};const char*nativeAimRows[13]={"SILENT AIM","FOV LIMIT","FOV RADIUS","TARGET PRIORITY","TARGET MARKER","TARGET SNAPLINE","CAMERA LOCK-ON KEY","LOCK-ON TARGET SOURCE","SMOOTHING","SPINBOT","SPIN SPEED","AIM BODY PART","CROSSHAIR"};const char*meleeRows[5]={"MELEE SILENT AIM","INFINITE DURABILITY","ONE-SHOT MELEE","CHAIN MELEE","EXPLOSIVE MELEE"};const char*nativeEspRows[9]={"ENEMY ESP","SKELETONS","BOXES","ALL HOSTILE SNAPLINES","MAX DISTANCE","REFRESH","RENDER STYLE","HEALTH BAR + HP","DISTANCE LABELS"};const char*styleRows[9]={"BACKGROUND","TEXT","ACCENT","BORDER","HIGHLIGHT","HEALTH BAR","ZOMBIE","FRIENDLY HUMAN","HOSTILE HUMAN"};const char*hotkeyRows[4]={"GOD MODE KEY","SKIP WAVE KEY","CANCEL WAVE KEY","MENU KEY"};const char*weaponRows[12]={"MAGAZINE AMMO","MAGAZINE CAPACITY","RESERVE AMMO","SHOT TYPE","SHOT/ANIMATION TYPE","FULL AUTO","FIRE INTERVAL","INTERVAL VALUE","RESTORE NATIVE GUN","ALWAYS CRITICAL","EXPLOSIVE SILENT AIM","NO RECOIL"};
 const char*visualRows[9]={"CHROMATIC ABERRATION","MOTION BLUR","DEPTH OF FIELD","FILM GRAIN","VIGNETTE","BLOOM / SUN SHAFTS","SUN GLARE","DISABLE ALL EFFECTS","RESTORE GAME EFFECTS"};
 const char*cameraRows[7]={"THIRD PERSON","IGNORE MAP BOUNDS","CUSTOM FOV","FIRST PERSON FOV","THIRD PERSON FOV","CAMERA DISTANCE","RESET CAMERA"};
 const char*funRows[10]={"HOMERUN KICKS","GROUND SLAM","CHAIN EXPLOSIONS","GRAVITY PUNCH","VEHICLE BOOST","GRAPPLE UPGRADES","GRAPPLE SPEED","GRAPPLE MOMENTUM","CHAOS HITS","DRIVE-BY GUNS"};
 const char*spawnRows[3]={"ITEM CATEGORY","SELECT ITEM","GIVE ONE ITEM"};
 const char*blueprintRows[5]={"SELECT BLUEPRINT","APPLY TO HELD WEAPON","REMOVE BLUEPRINT","REPAIR HELD WEAPON","BROWSE INVENTORY"};
 const char*playersRows[2]={"PLAYER TRACKER","YOUR POSITION"};
 const char**rows=mainRows;int count=5;if(page==1){rows=playerRows;count=8;}else if(page==3){rows=movementRows;count=7;}else if(page==2){rows=arenaRows;count=2;}else if(page==4){rows=nativeAimRows;count=13;}else if(page==5){rows=cameraRows;count=7;}else if(page==6){rows=nativeEspRows;count=9;}else if(page==7){rows=styleRows;count=9;}else if(page==8){rows=hotkeyRows;count=4;}else if(page==9){rows=weaponRows;count=12;}else if(page==10){rows=meleeRows;count=5;}else if(page==11){rows=blueprintRows;count=5;}else if(page==12){rows=spawnRows;count=3;}else if(page==13){rows=visualRows;count=9;}else if(page==14){rows=funRows;count=10;}else if(page==15){rows=configRows;count=4;}else if(page==17){rows=playersRows;count=2;}
 const int selected=std::clamp(menuIndex.load(),0,count-1);const float pw=510,panelH=h;const float rowH=MenuRowHeight(page);const float x=0,y=0;
 auto animation=MenuAnimation::state.Sample(GetTickCount64(),page,selected);
 MenuPanel(v,w,h,br,bg,bb,ar,ag,ab,tr,tg,tb);
 for(int t=0;t<MenuTabCount;t++){
  float tx=-194.f,ty=87.f+t*25.f;int id=MenuTabOrder[t];bool active=id==page;
  MenuRound(v,tx,ty,176,23,w,h,active?hr:.12f,active?hg:.085f,active?hb:.17f,1);
  if(active)AddRect(v,tx,ty+4,2,15,w,h,ar,ag,ab,1);
  AddText(v,tx+12,ty+8,pages[id],1.05f,w,h,active?tr:.75f,active?tg:.67f,active?tb:.85f);
 }

 AddText(v,22,92,page==11?"WEAPON MODIFICATION":page==12?"ITEM GIVER":pages[page],1,w,h,ar,ag,ab);
 const size_t contentVertex=v.size();
 for(int i=0;i<count;i++)MenuRound(v,10,101+i*rowH,pw-20,rowH-2,w,h,br+.025f,bg+.022f,bb+.025f,1);
 float selectionY=101+animation.row*rowH;MenuHighlight(v,10,selectionY,pw-20,rowH-3,w,h,GetTickCount64());
 for(int i=0;i<count;i++){float ry=y+104+i*rowH;float labelLimit=(page==12&&i==0)?120.f:MenuAdjustable(page,i)?(page==5&&(i==3||i==4)?198.f:294.f):348.f;float labelScale=std::min(1.3f,labelLimit/(6.f*strlen(rows[i])));AddText(v,x+22,ry+1,rows[i],labelScale,w,h,tr,tg,tb);char value[32]{};float valueX=x+385;if(page==1&&i==7)strcpy_s(value,VisualToggles::ready?(VisualToggles::infiniteFlashlight?"ON":"OFF"):"N/A");else if(page==1&&i==6)strcpy_s(value,ChapterUnlock::enabled?"ON":ChapterUnlock::ready?"OFF":"N/A");else if(page==12&&i==0){strcpy_s(value,SpawnCategoryNames[spawnCategory.load()]);valueX=x+385;}else if(page==12&&i==1)sprintf_s(value,"%d / %d",spawnSelection.load()-SpawnCategoryStarts[spawnCategory.load()]+1,SpawnCategoryCounts[spawnCategory.load()]);else if(page==11&&i==0)sprintf_s(value,"%d / %d",blueprintSelection.load()+1,BlueprintCount);else if(page==1&&i==0)strcpy_s(value,godModeEnabled?"ON":"OFF");else if(page==3&&i==0)strcpy_s(value,staminaReady.load()?(staminaOn.load()?"ON":"OFF"):"N/A");else if(page==3&&i==4)strcpy_s(value,doubleJumpReady.load()?(doubleJumpOn.load()?"ON":"OFF"):"N/A");else if(page==3&&i==5)strcpy_s(value,noclipReady.load()?(noclipOn.load()?"ON":"OFF"):"N/A");else if(page==3&&i==6){if(noclipReady.load())sprintf_s(value,"%.1f",kNoclipBaseSpeed.load());else strcpy_s(value,"N/A");}else if(page==3&&i>=1&&i<=3){bool ready=i==1?jumpReady.load():i==2?runReady.load():swingReady.load();if(ready)sprintf_s(value,"%.2f X",i==1?jumpScale.load():i==2?runScale.load():swingScale.load());else strcpy_s(value,"N/A");}else if(page==1&&i==1)sprintf_s(value,"%d",customCash.load());else if(page==4&&i==0)strcpy_s(value,enabled?"ON":"OFF");else if(page==4&&i==1)strcpy_s(value,fovAimEnabled?"ON":"OFF");else if(page==4&&i==2){sprintf_s(value,"%d",fovRadiusPixels.load());valueX=x+430;}else if(page==4&&i==3)strcpy_s(value,AimTargeting::PriorityLabel());else if(page==4&&i==4)strcpy_s(value,targetMarkerEnabled?"ON":"OFF");else if(page==4&&i==5)strcpy_s(value,targetSnaplineEnabled?"ON":"OFF");else if(page==4&&i==9)strcpy_s(value,Spinbot::enabled?"ON":"OFF");else if(page==4&&i==10)sprintf_s(value,"%d X",Spinbot::speed.load());else if(page==4&&i==11)strcpy_s(value,AimTargeting::PartLabel());else if(page==4&&i==7)strcpy_s(value,CameraLockOn::targetSource.load()?"ZOMBIES ONLY":"HOSTILE ROSTER");else if(page==4&&i==8)sprintf_s(value,"%d",CameraLockOn::smoothing.load());else if(page==4&&i==12)strcpy_s(value,Crosshair::enabled.load()?"ON":"OFF");else if(page==10&&i==0)strcpy_s(value,!meleeMenuState||meleeMenuState()<0?"N/A":meleeMenuState()==1?"ON":"OFF");else if(page==10&&i==1)strcpy_s(value,!wearMenuState||wearMenuState()<0?"N/A":wearMenuState()==1?"ON":"OFF");else if(page==10&&i==2)strcpy_s(value,!damageReady.load()||!oneShotMenuState||oneShotMenuState()<0?"N/A":oneShotMenuState()==1?"ON":"OFF");else if(page==10&&i==4)strcpy_s(value,!explosiveMeleeReady.load()?"N/A":explosiveMeleeOn.load()?"ON":"OFF");else if(page==10&&i==3)strcpy_s(value,!chainMenuState||chainMenuState()<0?"N/A":chainMenuState()==1?"ON":"OFF");else if(page==6&&i<3)strcpy_s(value,(i==0?espEnabled.load():i==1?espSkeletonsEnabled.load():espBoxesEnabled.load())?"ON":"OFF");else if(page==6&&i==3)strcpy_s(value,espSnaplinesEnabled?"ON":"OFF");else if(page==6&&i==4)sprintf_s(value,"%d M",control?control->maxDistanceTenths/10:400);else if(page==6&&i==5)sprintf_s(value,"%d MS",control?control->skeletonIntervalMs:8);else if(page==6&&i==6)sprintf_s(value,"%d",control?control->styleScaleHundredths/10:0);else if(page==6&&i>=7)strcpy_s(value,(i==7?espHealthEnabled.load():espDistanceEnabled.load())?"ON":"OFF");else if(page==8){int key=hotkeys[i].load();sprintf_s(value,"%d",key);}
  else if(page==5){if(i==0)strcpy_s(value,!CameraFeatures::ready?"N/A":CameraFeatures::third?(CameraFeatures::active?"ON":"WAIT"):"OFF");else if(i==1)strcpy_s(value,!CameraFeatures::boundsReady?"N/A":CameraFeatures::bounds?"ON":"OFF");else if(i==2)strcpy_s(value,!CameraFeatures::ready?"N/A":CameraFeatures::customFov?"ON":"OFF");else if(i==3||i==4)sprintf_s(value,"%d DEG",i==3?CameraFeatures::firstFov.load():CameraFeatures::thirdFov.load());else if(i==5)sprintf_s(value,"%.2f M",CameraFeatures::distanceCm.load()*.01f);else strcpy_s(value,"APPLY");}
  else if(page==13&&i<7)strcpy_s(value,GraphicsToggles::Ready(i)?((GraphicsToggles::disabled.load()&(1u<<i))?"OFF":"GAME"):"N/A");
   else if(page==14){if(!FunReady(i))strcpy_s(value,"N/A");else if(i==6)sprintf_s(value,"%.0f",grappleSpeed.load());else if(auto toggle=FunToggle(i))strcpy_s(value,toggle->load()?"ON":"OFF");}
   else if(page==17&&i==0)strcpy_s(value,PlayerTracker::enabled.load()?"ON":"OFF");
   else if(page==17&&i==1){PlayerTracker::GetLocalPlayerCoords(value,sizeof(value));valueX=x+330;}
   MenuValue(v,valueX,ry,value,w,h,ar,ag,ab);}
 for(int i=0;i<count;i++)if(page!=8){
  int id=FeatureBindings::BindingId(page,i);char key[32]{},label[48]{};FeatureBindings::KeyName(FeatureBindings::keys[id],key,sizeof(key));sprintf_s(label,"KEY: %s",key);
  float yy=118.f+i*rowH;MenuRound(v,22,yy,142,11,w,h,.19f,.12f,.14f,1);AddText(v,27,yy+2,captureHotkey.load()==id?"PRESS KEY...":label,.85f,w,h,.82f,.75f,.73f);
  MenuRound(v,169,yy,48,11,w,h,.25f,.12f,.14f,1);AddText(v,175,yy+2,"RESET",.85f,w,h,.94f,.76f,.72f);
 }
 if(page==15){for(int i=0;i<4;i++){float yy=103.f+i*rowH;MenuRound(v,326,yy,66,17,w,h,.29f,.16f,.40f,1);AddText(v,340,yy+4,"LOAD",1,w,h,tr,tg,tb);MenuRound(v,414,yy,75,17,w,h,.19f,.13f,.28f,1);AddText(v,425,yy+4,i?"SAVE":"LOCKED",1,w,h,tr,tg,tb);}AddText(v,22,255,"DEFAULT CANNOT BE OVERWRITTEN",1,w,h,tr,tg,tb);AddText(v,22,278,"PRESETS SAVE SETTINGS AND KEY BINDINGS",1,w,h,tr,tg,tb);AddText(v,22,301,"LOADING DOES NOT REPLAY SPAWN OR CASH ACTIONS",1,w,h,tr,tg,tb);}
 if(page==6){AddText(v,22,389,"HP* BAR USES HIGHEST OBSERVED HEALTH",1,w,h,tr,tg,tb);AddText(v,22,409,"HEALTH AND DISTANCE CAN BE TOGGLED SEPARATELY",1,w,h,.64f,.59f,.59f);}
 if(page==7){for(int i=0;i<9;i++){unsigned color=i<6?(unsigned)themeColors[i]:control?(unsigned)(i==6?control->zombieArgb:i==7?control->friendlyArgb:control->hostileArgb):0;MenuRound(v,424,104+i*rowH,56,18,w,h,((color>>16)&255)/255.f,((color>>8)&255)/255.f,(color&255)/255.f,1);}}
 if(page==13){AddText(v,22,419,"GAME = YOUR NORMAL GRAPHICS SETTINGS",1,w,h,tr,tg,tb);AddText(v,22,440,"OFF = EFFECT SUPPRESSED WHILE PLAYING",1,w,h,tr,tg,tb);}
 if(page==5){for(int row=3;row<=4;row++){float yy=108+row*rowH;float fraction=((row==3?CameraFeatures::firstFov.load():CameraFeatures::thirdFov.load())-50)/70.f;AddRect(v,230,yy+3,88,5,w,h,.25f,.28f,.3f,1);AddRect(v,230,yy+3,88*fraction,5,w,h,ar,ag,ab,1);AddRect(v,228+88*fraction,yy,4,11,w,h,tr,tg,tb,1);}AddText(v,22,337,"FOV: DRAG BAR OR USE WHEEL / +/-",1,w,h,tr,tg,tb);AddText(v,22,360,"OFF RESTORES THE NATIVE CAMERA FOV",1,w,h,tr,tg,tb);if(CameraFeatures::fault)AddText(v,22,386,"CAMERA CHECK FAILED - SEE SESSION LOG",1,w,h,1,.4f,.2f);}
 if(page==2){char info[96]{};int wave=waveDisplayed.load();sprintf_s(info,wave?"CURRENT WAVE: %d":"NO ACTIVE ARENA WAVE",wave);AddText(v,22,218,info,1.5f,w,h,tr,tg,tb);AddText(v,22,250,"PRESS ONCE THEN RESUME GAME",1.2f,w,h,tr,tg,tb);AddText(v,22,276,"CANCEL STOPS REMAINING CONTROLLERS",1,w,h,tr,tg,tb);}
 if(page==12){char label[96]{};ReadableWeaponLabel(SpawnIds[std::clamp(spawnSelection.load(),0,SpawnCount-1)],label);label[64]=0;AddText(v,22,410,"ITEM:",1,w,h,ar,ag,ab);AddText(v,64,410,label,std::min(1.f,430.f/(6.f*std::max<size_t>(1,strlen(label)))),w,h,tr,tg,tb);AddText(v,22,450,"SELECT A CATEGORY AND ITEM, THEN GIVE ONE",1,w,h,.73f,.65f,.86f);}

 if(page==11){
  AddText(v,22,268,"SELECTED BLUEPRINT",1.2f,w,h,ar,ag,ab);
  char selectedLabel[96]{};ReadableWeaponLabel(BlueprintIds[std::clamp(blueprintSelection.load(),0,BlueprintCount-1)],selectedLabel);AddText(v,22,286,selectedLabel,1.1f,w,h,tr,tg,tb);
  HeldBlueprintState held{};char name[96]{},attached[96]{};
  if(ResolveHeldBlueprint(held)&&BlueprintName(held.base,name)){
   AddText(v,22,312,"HELD WEAPON",1.2f,w,h,ar,ag,ab);char weaponLabel[96]{};ReadableWeaponLabel(name,weaponLabel);weaponLabel[55]=0;AddText(v,22,332,weaponLabel,1,w,h,tr,tg,tb);
   if(!held.blueprint||!BlueprintName(held.blueprint,attached))strcpy_s(attached,"NONE");char attachedLabel[96]{};ReadableWeaponLabel(attached,attachedLabel);attachedLabel[55]=0;
   AddText(v,22,356,"ATTACHED BLUEPRINT",1.2f,w,h,ar,ag,ab);AddText(v,22,376,attachedLabel,1,w,h,tr,tg,tb);
  char conditionText[80]{};sprintf_s(conditionText,"CONDITION %.2f   QUANTITY %d",held.condition,held.quantity);AddText(v,22,400,conditionText,1.f,w,h,tr,tg,tb);
  }else AddText(v,22,312,"EQUIP A GUN OR MELEE WEAPON",1.2f,w,h,tr,tg,tb);
 }
 if(page==9)for(int i=0;i<12;i++){char value[32]{};if(i<3&&ammoEditField.load()==i)sprintf_s(value,ammoEditHasDigits.load()?">%d_":">_",ammoEditValue.load());else if(i<3&&ammoDisplayValid.load()){const int displayed[3]={ammoDisplayLoaded.load(),ammoDisplayCapacity.load(),ammoDisplayReserve.load()};sprintf_s(value,"%d",displayed[i]);}else if(i<3)strcpy_s(value,"N/A");else if(i==3){static const char*shotLabels[4]={"NORMAL","SHOTGUN","ROCKET","RIFLE"};strcpy_s(value,shotLabels[std::clamp(weaponShotTypeMode.load(),0,3)]);}else if(i==4){static const char*profileLabels[4]={"NORMAL","PISTOL","RIFLE","SHOTGUN"};strcpy_s(value,profileLabels[std::clamp(weaponProfileType.load(),0,3)]);}else if(i==5)strcpy_s(value,weaponFullAutoEnabled.load()?"ON":"OFF");else if(i==6)strcpy_s(value,weaponIntervalEnabled.load()?"ON":"OFF");else if(i==8)strcpy_s(value,"APPLY");else if(i==9)strcpy_s(value,alwaysCriticalEnabled.load()?"ON":"OFF");else if(i==10)strcpy_s(value,explosiveSilentAimEnabled.load()?"ON":"OFF");else if(i==11)strcpy_s(value,NoRecoil::enabled.load()?"ON":"OFF");else{sprintf_s(value,"%.2f S",weaponIntervalHundredths.load()/100.f);}MenuValue(v,x+382,y+108+i*rowH,value,w,h,ar,ag,ab);}
 AddRect(v,16,h-52,478,1,w,h,.31f,.20f,.42f,1);AddText(v,x+16,y+h-40,menuStatus,std::min(1.f,478.f/(6.f*std::max<size_t>(1,strlen(menuStatus)))),w,h,.83f,.74f,.94f);AddText(v,x+16,y+h-22,ammoEditField.load()>=0?"DIGITS TYPE   BACKSPACE EDIT   ENTER APPLY   ESC CANCEL":"",1,w,h,.60f,.55f,.55f);
for(int i=0;i<count;i++)if(MenuAdjustable(page,i)){float ry=101.f+i*rowH,boxH=std::min(25.f,rowH-1.f);AddRect(v,326,ry,24,boxH,w,h,.26f,.16f,.36f,1);AddText(v,334,ry+8,"-",1.5f,w,h,tr,tg,tb);AddRect(v,354,ry,24,boxH,w,h,.26f,.16f,.36f,1);AddText(v,362,ry+8,"+",1.5f,w,h,tr,tg,tb);}
 if(ammoEditField.load()>=0){AddRect(v,330,390,80,22,w,h,.15f,.32f,.2f,1);AddText(v,342,397,"APPLY",1.5f,w,h,tr,tg,tb);AddRect(v,416,390,80,22,w,h,.35f,.15f,.18f,1);AddText(v,429,397,"CANCEL",1.5f,w,h,tr,tg,tb);}
 float mx=0,my=0;if(MenuMousePosition(screenW,screenH,mx,my)){AddRect(v,mx,my,2,10,w,h,1,1,1,1);AddRect(v,mx,my,7,2,w,h,1,1,1,1);}
 auto layout=MenuLayoutFor(screenW,screenH);
 for(size_t i=firstVertex;i<v.size();i++){auto&pt=v[i];pt.a*=animation.opacity*(i>=contentVertex?animation.content:1.f);float px=(pt.x+1)*255+210,py=(1-pt.y)*270;pt.x=(layout.x+px*layout.scale)*2/screenW-1;pt.y=1-(layout.y+py*layout.scale)*2/screenH;}
}
...
void DrawNativeEsp(IDXGISwapChain *sc) {
  const bool aimActive=enabled.load(std::memory_order_acquire),drawEsp=espEnabled.load(std::memory_order_acquire),drawMenu=menuOpen.load(std::memory_order_acquire),drawFov=fovAimEnabled.load(std::memory_order_acquire),drawTargetMarker=aimActive&&targetMarkerEnabled.load(std::memory_order_acquire),drawTargetLine=aimActive&&targetSnaplineEnabled.load(std::memory_order_acquire),drawCrosshair=Crosshair::enabled.load(std::memory_order_acquire),drawPlayerTracker=PlayerTracker::enabled.load(std::memory_order_acquire);
  if(!drawMenu)MenuAnimation::state.Close();
  if (!drawEsp&&!drawMenu&&!drawFov&&!drawTargetMarker&&!drawTargetLine&&!drawCrosshair&&!drawPlayerTracker)
    return;
  if (!ForegroundGame()) {
  ...
  EnsureMenuWindow(sd.OutputWindow);
  ...
  if(drawEsp){V pos{}, right{}, up{}, back{};float sx=0,sy=0;EspFrame frame{};
   ...
   for (unsigned i = 0; i < frame.count; i++) {
    ...
    if (!ValidateEspEntity(frame.entities[i], root, &currentHealth)) {
    ...
    if (espSkeletonsEnabled.load(std::memory_order_relaxed))
      rendered = AppendSkeleton(verts, frame.entities[i], observedRoot, now, pos, right,
                                ...
                                baseThickness, style);
    if (espBoxesEnabled.load(std::memory_order_relaxed)||espHealthEnabled.load(std::memory_order_relaxed)||espDistanceEnabled.load(std::memory_order_relaxed)) {
      ...
      if (ComputeEspBox(frame.entities[i], displayRoot, now, pos, right, up, back, sx,
        ...
        rendered = AppendEspVitals(verts,frame.entities[i],currentHealth,distance,now,boxLeft,boxTop,boxRight,boxBottom,w,h)||rendered;
      }
    }
  ...
  if(drawCrosshair && !drawMenu){Crosshair::Draw(verts,w,h);}
  if(drawPlayerTracker && !drawMenu){PlayerTracker::Draw(verts,w,h);}
  ...
  if (verts.empty() || verts.size()>131072) {
    ctx->Release();
    d->Release();
    return;
  }
  ...
  ID3D11ShaderResourceView *oldFontTexture=nullptr;ID3D11SamplerState *oldFontSampler=nullptr;
  ...
  ctx->PSGetShaderResources(0,1,&oldFontTexture);ctx->PSGetSamplers(0,1,&oldFontSampler);
  ...
  ctx->PSSetShaderResources(0,1,&SmoothFont::texture);ctx->PSSetSamplers(0,1,&SmoothFont::sampler);
  ...
  ctx->PSSetShaderResources(0,1,&oldFontTexture);ctx->PSSetSamplers(0,1,&oldFontSampler);if(oldFontTexture)oldFontTexture->Release();if(oldFontSampler)oldFontSampler->Release();
...
void HandleMenuAction(int page,int index,bool activate,bool decrease){
 if(page==14)return;
 if(!FeatureBindings::Exists(FeatureBindings::BindingId(page,index)))return;
  if(page==0&&index==0&&activate){MenuPresets::Load(0);}

  else if(page==0&&index==1&&activate){menuOpen=false;stop=true;Log("UNLOAD_MENU");}
  else if(page==0&&index==2&&activate){RoutineDiagnostics::Emit("USER_ISSUE_MARKER inspect_previous_and_next_events");RoutineDiagnostics::Pump(true);SetStatus("ISSUE MARKED IN LOG");}
  else if(page==0&&index==3&&activate){RoutineDiagnostics::Pump(true);if(RoutineDiagnostics::ready.load()){auto result=ShellExecuteW(nullptr,L"open",RoutineDiagnostics::folder.c_str(),nullptr,nullptr,SW_SHOWNORMAL);SetStatus((INT_PTR)result>32?"LOG FOLDER OPENED":"COULD NOT OPEN LOG FOLDER");}else SetStatus("LOGGER UNAVAILABLE");}
  else if(page==1&&index==6&&(activate||decrease))ChapterUnlock::Set(!ChapterUnlock::enabled.load());
  ...
  else if(page==1&&index==2&&activate)GiveCashNative(customCash.load());
  else if(page==1&&index==3&&activate)GiveCashNative(1000);
  else if(page==1&&index==4&&activate)GiveCashNative(200000);
  else if(page==1&&index==5&&activate){unsigned char bytes[8]{};ReadBytes(gameBase+GodModeRva,bytes,8);static const unsigned char nops[8]={0x90,0x90,0x90,0x90,0x90,0x90,0x90,0x90},godOriginal[8]={0xF3,0x0F,0x11,0x8A,0x2C,0x0D,0,0};if(!memcmp(bytes,nops,8)&&WriteCode(gameBase+GodModeRva,godOriginal,8)){godModeEnabled=false;SetStatus("GOD BYTES RESTORED");}else SetStatus("RESTORE REFUSED - NOT OUR PATCH");}
  else if(page==3&&index>=1&&index<=3&&(activate||decrease)){
   bool ready=index==1?jumpReady.load():index==2?runReady.load():swingReady.load();
   if(ready){auto&setting=index==1?jumpScale:index==2?runScale:swingScale;setting=std::clamp(setting.load()+(decrease?-.25f:.25f),.25f,index==1?20.f:10.f);SetStatus("MODIFIER UPDATED - 1 X IS NORMAL");}else SetStatus("NATIVE MODIFIER UNAVAILABLE");
  }
  else if(page==3&&index==4&&(activate||decrease)){if(doubleJumpReady){SetDoubleJump(!doubleJumpOn.load());SetStatus(doubleJumpOn?"INFINITE AIR JUMPS ENABLED":"INFINITE AIR JUMPS DISABLED");}}
  else if(page==3&&index==5&&(activate||decrease)){if(noclipReady.load())ToggleNoclip();else SetStatus("NOCLIP UNAVAILABLE");}
  else if(page==3&&index==6&&(activate||decrease)){if(noclipReady.load())AdjustNoclipSpeed(activate?1:-1);else SetStatus("NOCLIP UNAVAILABLE");}
  else if(page==3&&index==5&&(activate||decrease)){if(noclipReady.load())ToggleNoclip();else SetStatus("NOCLIP UNAVAILABLE");}
  else if(page==3&&index==6&&(activate||decrease)){if(noclipReady.load())AdjustNoclipSpeed(activate?1:-1);else SetStatus("NOCLIP UNAVAILABLE");}
  ...
  else if(page==4&&index==12&&(activate||decrease)){Crosshair::Toggle();}
  else if(page==17&&index==0&&(activate||decrease)){PlayerTracker::Toggle();}
  ...
  else if(page==12&&index==2&&activate)QueueSpawnItem();
  else if(page==11&&index==0&&(activate||decrease)){blueprintSelection=(blueprintSelection.load()+(activate?1:BlueprintCount-1))%BlueprintCount;}
  else if(page==11&&index==1&&activate)QueueBlueprintChange(false);
  else if(page==11&&index==2&&activate)QueueBlueprintChange(true);
  else if(page==11&&index==3&&activate)QueueHeldRepair();
  else if(page==11&&index==4&&(activate||decrease))BrowseInventory(activate?1:-1);
  else if(page==10&&index==0&&(activate||decrease))ToggleMeleeMenu();
  else if(page==10&&index==1&&(activate||decrease))ToggleWearMenu();
  else if(page==10&&index==2&&(activate||decrease)){if(damageReady)ToggleOneShotMelee();else SetStatus("MELEE DAMAGE HOOK UNAVAILABLE");}
  else if(page==10&&index==3&&(activate||decrease))ToggleChainMelee();
  else if(page==10&&index==4&&(activate||decrease))ToggleExplosiveMelee();
  else if(page==1&&index==7&&(activate||decrease)){if(VisualToggles::ready){VisualToggles::infiniteFlashlight=!VisualToggles::infiniteFlashlight.load();SetStatus("FLASHLIGHT SETTING UPDATED");}}
  else if(page==13&&(activate||decrease)){if(index<7){GraphicsToggles::Toggle(index);SetStatus("GRAPHICS SETTING UPDATED");}else if(index==7){GraphicsToggles::DisableAll();SetStatus("EFFECT SUPPRESSION ENABLED");}else if(index==8){GraphicsToggles::Reset();SetStatus("GAME EFFECTS RESTORED");}}
  else if(page==14&&index<10&&(activate||decrease))AdjustFunMenu(index,activate);
  ...
  else if(page==6&&index>=7&&(activate||decrease)){auto&setting=index==7?espHealthEnabled:espDistanceEnabled;setting=!setting.load();SetStatus(index==7?"BAR USES HIGHEST OBSERVED HP":"CAMERA DISTANCE LABEL UPDATED");}
  ...
  else if(page==0&&index==4&&activate){menuOpen=!menuOpen.load();}
  else if(page==15&&(activate||decrease)){if(decrease)MenuPresets::Save(index);else MenuPresets::Load(index);}
 else if(page==9&&index<3&&activate){menuOpen=true;menuPage=9;menuIndex=index;BeginAmmoEdit(index);}
  ...
  else if(page==9&&index==8&&activate){NoRecoil::Set(false);RestoreNativeGunStyles();}
  else if(page==9&&index==11&&(activate||decrease))NoRecoil::Set(!NoRecoil::enabled.load());
  else if(page==9&&index==9&&(activate||decrease))SetAlwaysCritical(!alwaysCriticalEnabled.load());
  else if(page==9&&index==10&&(activate||decrease))SetExplosiveSilentAim(!explosiveSilentAimEnabled.load());
  if((activate||decrease)&&page==6)SaveNativeSettings();
}
void HandleMenuInput(){
 TrueGodTick();WaveMenuTick();
 SyncMenuCursor();PollMenuButtons();
 if(!ForegroundGame()){FeatureBindings::Poll(false);return;}
 ...
 if(captureHotkey.load()>=0){HandleBindingCapture();return;}
 if(FeatureBindings::suppressUntilRelease){bool any=false;for(int k=1;k<255;k++)any|=(GetAsyncKeyState(k)&0x8000)!=0;if(!any)FeatureBindings::suppressUntilRelease=false;return;}
 static bool ammoReady=false;
 if(ammoEditField.load()>=0){int click=menuMouseClick.exchange(0);float mx=0,my=0;if(click&&MenuMousePosition((float)presentWidth.load(),(float)presentHeight.load(),mx,my)&&my>=390&&my<412){if(mx>=330&&mx<410){ammoReady=false;CommitAmmoEdit();return;}if(mx>=416&&mx<496){ammoReady=false;CancelAmmoEdit();return;}}bool enter=(GetAsyncKeyState(VK_RETURN)&0x8000)!=0,escape=(GetAsyncKeyState(VK_ESCAPE)&0x8000)!=0,back=(GetAsyncKeyState(VK_BACK)&0x8000)!=0,any=enter||escape||back;int digit=-1;for(int i=0;i<10;i++)if((GetAsyncKeyState('0'+i)&0x8000)||(GetAsyncKeyState(VK_NUMPAD0+i)&0x8000)){any=true;digit=i;break;}if(!ammoReady){if(!any)ammoReady=true;return;}if(escape){ammoReady=false;CancelAmmoEdit();return;}if(back){ammoEditValue=ammoEditValue.load()/10;ammoEditHasDigits=ammoEditValue.load()!=0;ammoReady=false;return;}if(digit>=0){int before=ammoEditValue.load();if(before<214748364||(before==214748364&&digit<=7)){ammoEditValue=before*10+digit;ammoEditHasDigits=true;}else SetStatus("AMMO VALUE TOO LARGE");ammoReady=false;return;}if(enter){ammoReady=false;CommitAmmoEdit();return;}return;}
 static unsigned long long nextAmmoRefresh=0,nextModifierRefresh=0,nextFovSync=0;auto inputNow=GetTickCount64();if(inputNow>=nextModifierRefresh){MaintainWeaponModifiers();nextModifierRefresh=inputNow+100;}if(menuOpen.load()&&menuPage.load()==9&&inputNow>=nextAmmoRefresh){RefreshWeaponAmmo();nextAmmoRefresh=inputNow+100;}if(inputNow>=nextFovSync){if(meleeFovSync)meleeFovSync(fovAimEnabled.load()?1:0,fovRadiusPixels.load(),(int)presentWidth.load(),(int)presentHeight.load());nextFovSync=inputNow+100;} const bool menuKey=(GetAsyncKeyState(hotkeys[3].load())&0x8000)!=0,up=(GetAsyncKeyState(VK_UP)&0x8000)!=0,down=(GetAsyncKeyState(VK_DOWN)&0x8000)!=0,left=(GetAsyncKeyState(VK_LEFT)&0x8000)!=0,right=(GetAsyncKeyState(VK_RIGHT)&0x8000)!=0,enter=(GetAsyncKeyState(VK_RETURN)&0x8000)!=0,pgup=(GetAsyncKeyState(VK_PRIOR)&0x8000)!=0,pgdown=(GetAsyncKeyState(VK_NEXT)&0x8000)!=0;
 if(menuKey&&!oldMenu){menuMouseClick=0;menuMouseWheel=0;menuOpen=!menuOpen.load();Log(menuOpen?"MENU_OPEN":"MENU_CLOSED");}
 ...
 int boundAction=FeatureBindings::Poll(!menuOpen.load()&&ammoEditField.load()<0&&captureHotkey.load()<0);if(boundAction>=0){if(boundAction>=260)MenuPresets::Save(boundAction-259);else HandleMenuAction(boundAction/16,boundAction%16,true,false);char record[100]{};sprintf_s(record,"HOTKEY_ACTION id=%d key=%d",boundAction,FeatureBindings::keys[boundAction].load());RoutineDiagnostics::Emit(record);}
 if(menuOpen.load()){
  int page=menuPage.load();if(pgup&&!oldPgUp){page=MenuNextTab(page,-1);menuPage=page;menuIndex=0;}if(pgdown&&!oldPgDown){page=MenuNextTab(page,1);menuPage=page;menuIndex=0;}const int counts[18]={5,8,2,7,13,7,9,9,4,12,5,5,3,9,10,4,0,2};int count=counts[page],index=menuIndex.load();if(up&&!oldUp)index=(index+count-1)%count;if(down&&!oldDown)index=(index+1)%count;menuIndex=index;
  bool mouseActivate=false,mouseDecrease=false;
  int click=menuMouseClick.exchange(0),wheel=menuMouseWheel.exchange(0);float mx=0,my=0;
  if((click||wheel)&&MenuMousePosition((float)presentWidth.load(),(float)presentHeight.load(),mx,my)){
   if(click&&mx>=475&&mx<501&&my>=8&&my<34){menuOpen=false;}
   else if(click&&mx>=-194&&mx<-18&&my>=87&&my<87+25*MenuTabCount){int tab=(int)((my-87)/25);page=MenuTabOrder[tab];menuPage=page;index=0;menuIndex=0;Log("MOUSE_MENU_TAB_SELECTED");}
   else if(mx>=10&&mx<500&&my>=101&&my<101+counts[page]*MenuRowHeight(page)){index=(int)((my-101)/MenuRowHeight(page));menuIndex=index;Log("MOUSE_MENU_ROW_SELECTED");float within=my-(101+index*MenuRowHeight(page));if(click==1&&FeatureBindings::HitBar(mx,within)){int id=FeatureBindings::BindingId(page,index);if(FeatureBindings::HitBar(mx,within)==2){int key=id==256?VK_INSERT:0;if(FeatureBindings::Assign(id,key)){hotkeys[3]=FeatureBindings::keys[256].load();MenuPresets::SaveBindings();SetStatus("HOTKEY RESET");}else SetStatus("RESET CONFLICT - UNBIND INSERT FIRST");}else{captureHotkey=id;FeatureBindings::captureReleased=false;SetStatus("RELEASE KEYS THEN PRESS KEY - ESC CANCELS");}return;}mouseDecrease=(page==15&&click==1&&mx>=414)||click==2||wheel<0||(MenuAdjustable(page,index)&&mx>=326&&mx<350);mouseActivate=!mouseDecrease&&(click||wheel>0);}
  }
  if(page==5&&MenuMousePosition((float)presentWidth.load(),(float)presentHeight.load(),mx,my)&&mx>=230&&mx<=318&&((GetAsyncKeyState(VK_LBUTTON)&0x8000)||menuLeftHeld.load()||click==1)){int row=(int)((my-101)/MenuRowHeight(page));if(my>=101&&(row==3||row==4)){int degree=(int)std::lround(50+std::clamp((mx-230)/88.f,0.f,1.f)*70);(row==3?CameraFeatures::firstFov:CameraFeatures::thirdFov)=degree;menuIndex=row;index=row;mouseActivate=false;mouseDecrease=false;}}
  const bool activate=mouseActivate||(enter&&!oldEnter)||(right&&!oldRight),decrease=mouseDecrease||(left&&!oldLeft);
  if(activate||decrease)HandleMenuAction(page,index,activate,decrease);
 }
...
DWORD WINAPI Worker(void*){
 RoutineDiagnostics::Start("menu");RoutineDiagnostics::WorkerScope diagnosticScope;RoutineDiagnostics::Module(L"JentaSpecialEdition.dll","menu");RoutineDiagnostics::Module(L"gamedll_x64_rwdi.dll","game");RoutineDiagnostics::Module(L"engine_x64_rwdi.dll","engine");RoutineDiagnostics::Pump(true);
