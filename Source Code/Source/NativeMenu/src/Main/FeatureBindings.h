...
constexpr int groups[]={0,1,3,5,13,4,10,9,11,12,14,6,7,2,15,16};
constexpr const char* groupNames[]={"OVERVIEW","PLAYER","ARENA","MOVEMENT","AIM","CAMERA","ENEMY ESP","STYLE","HOTKEYS","FIREARMS","MELEE","CRAFTING","ITEMS","VISUALS","FUN","CONFIGS","MENU / ACTION KEYS"};
constexpr int rowCounts[]={5,8,2,5,12,7,9,9,0,12,5,5,3,9,10,4,7};
constexpr const char* labels[17][16]={
...
{"INFINITE STAMINA","JUMP HEIGHT","RUN SPEED","SWING SPEED","INFINITE AIR JUMPS"},
{"SILENT AIM","FOV LIMIT","FOV RADIUS","TARGET PRIORITY","TARGET MARKER","TARGET SNAPLINE","CAMERA LOCK-ON KEY","LOCK-ON TARGET SOURCE","SMOOTHING","SPINBOT","SPIN SPEED","AIM BODY PART"},
...
inline int Page(){return groups[std::clamp(group.load(),0,15)];}
constexpr bool Exists(int id){if(id==256||id==258||id==259)return true;return id>=0&&id<256&&id/16!=8&&id%16<rowCounts[id/16]&&BindingId(id/16,id%16)==id;}
